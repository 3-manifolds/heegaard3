...
/****************************** Functions in Heegaard10.c **********************************
L 1396 Cutting_Disk_Surgery_Diagram_7(void)
L 1759 Heegaard_Splash_Screen(void)
L 1516 Stabilize(char Flag)
L 1135 Try_Cutting_Disk_Surgery(void)
L  754 Try_Exponent_Surgery(void)
L   14 Wirtinger(void)
********************************************************************************************/
...
int Wirtinger(void)
{
	...
	*****************************************************************************************/	
	...
	/******************************************************************************************
		Deal with those relators which are of the form ABab or AbaB with A and B distinct.
	******************************************************************************************/

	for(i = 0; i < MAXNUMGENERATORS; i++) GL[i][4] = GL[i][5] = 150;
		...
		if(y < 'a')
			...
			if(TheCase == 1) /* There is a sublink which is a Hopf link. */				
				...
				if(GL[x][4] == 150)
					...
					{				
					if(GL[x][4] != y || GL[y][4] != x) return(23);
					p = *Relators[i];
					xx = *p++;
					yy = *p;
					q = *Relators[GL[x][5]];
					...
					ww = *q;
					F1 = FALSE;
					if(xx == ww && yy == zz) F1 = TRUE;
					if(abs(xx - ww) == 32 && abs(yy - zz) == 32) F1 = TRUE;
					if(F1)	
						{
						GL[x][1] = GL[x][2] = i;
						GL[x][0] = GL[y][0] = 100;
			...
			if(TheCase == 2)
				{
				if(GL[x][4] < 150 || GL[y][4] < 150) return(23);
				GL[x][1] = GL[x][2] = i;
				GL[x][3]++;				
				GL[x][0] = 100;
				}			
	...
	******************************************************************************************/	
		...
		for(i = 0; i < MAXNUMGENERATORS; i++) if(GL[i][3] == 1) return(10); 		
	...
	******************************************************************************************/
	...
	/**********************************************************************************************
				Finally, check for components which have no undercrossings.
	**********************************************************************************************/
	
	for(i = 1; i <= NumRelators; i++)
	for(p = *Relators[i]; *p != EOS; p++)
		{
		x = *p;	
		if(x < 'a' && GL[x - 'A'][0] == 0)
			{
			...
			GL[x - 'A'][0] = NumComponents;		
			if(WirtingerM[NumComponents] != NULL) DisposeHandle((char **) WirtingerM[NumComponents]);
			WirtingerM[NumComponents] = (unsigned char **) NewHandle(2*sizeof(char));
			if(WirtingerM[NumComponents] == NULL) Mem_Error();
			r = *WirtingerM[NumComponents];	
			*r++ = x;
			*r = EOS;
			
			if(WirtingerL[NumComponents] != NULL) DisposeHandle((char **) WirtingerL[NumComponents]);
			WirtingerL[NumComponents] = (unsigned char **) NewHandle(1*sizeof(char));
			if(WirtingerL[NumComponents] == NULL) Mem_Error();
			r = *WirtingerL[NumComponents];
			*r = EOS;
			}
...
int Try_Cutting_Disk_Surgery(void)
{
	/******************************************************************************************
...
int Cutting_Disk_Surgery_Diagram_7(void)
{
	/******************************************************************************************
...
void Heegaard_Splash_Screen(void)
{
