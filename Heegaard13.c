...
/****************************** Functions in Heegaard13.c **********************************
L  450 Do_LT(unsigned int Source,int F1,int F2)
L   21 Find_Level_Transformations(int F1,int F2)
L  132 Find_Level_Flow(unsigned int Source)
L  383 Get_LTs(unsigned int Source,int F1,int F2)
L  214 Init_SCdfsR(void)
L  303 Init_TCR(void)
L  257 SCdfsR(unsigned char w)
L  354 TCdfsR(unsigned char v)
********************************************************************************************/
...
{
    /******************************************************************************************
        This routine is a variant of Find_Flow_A() which, unlike Find_Flow_A(), only finds a 
        maximal flow in the Whitehead graph between a single pair of vertices, namely the 
        source and sink passed to it. (Note that we reverse the roles of Source and Sink 
        here because it makes things slightly more convenient for following routines.)
    ******************************************************************************************/
...
void Init_TCR(void) 	
{
	...
	***********************************************************************************/
	...
	of G'. Then set each diagonal element of TC[][] equal to 1 so that each 
	vertex of G' lies in its transititive closure.
	*****************************************************************************/
