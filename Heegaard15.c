...
int Do_Initialization()
{
	...
	for(i = 0; i < 2*MAXNUMRELATORS; i++)
		{
		DRA[i] = (unsigned int *) NewPtr(sizeof(int)*(2*MAXNUMRELATORS + 1));
	...
	Alphas			= (         int  *)  NewPtr(sizeof(int)*(MAXNUMGENERATORS + 1));
	if(Alphas		== NULL) Mem_Error();
	...
	Betas			= (         int  *)  NewPtr(sizeof(int)*(MAXNUMGENERATORS + 1));
	if(Betas		== NULL) Mem_Error();
	...
	SETR1			= (unsigned char **) NewPtr(sizeof(long)*(MAXNUMCOMPONENTS + 1));
	if(SETR1	 	== NULL) Mem_Error();
	SETR2			= (unsigned char **) NewPtr(sizeof(long)*(MAXNUMCOMPONENTS + 1));
	if(SETR2	 	== NULL) Mem_Error();
	SETR3			= (unsigned char **) NewPtr(sizeof(long)*(MAXNUMCOMPONENTS + 1));
	if(SETR3	 	== NULL) Mem_Error();
	SETR4			= (unsigned char **) NewPtr(sizeof(long)*(MAXNUMCOMPONENTS + 1));
	if(SETR4	 	== NULL) Mem_Error();		
	...
	Table1 			= (			int*  ) NewPtr((sizeof(int)*MAX_SAVED_PRES));
    if(Table1 		== NULL) Mem_Error();
