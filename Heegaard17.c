...
int Just_Delete_Primitives(char F1,char F2)
{
	...
	/**********************************************************************************************
			Just_Delete_Primitives() produces a set of presentations obtained from an initial
		presentation by successively deleting primitive relators until all relators of all 
		presentations produced have been checked for primitivity, or until a simple recognizable
		presentation, like e.g. a presentation on n generators and 0 relators of F_n.
			Orbits under level-transformations are not computed, so the routine may save
		distinct presentations that actually lie in the same orbit under level-transformations.
		Because of this, and randomization done, successive runs of the routine may result in 
		lists containing different presentations and different numbers of presentations. These
		differences should disappear when orbits under level-transformations are taken into
		account.
			No realizability checking is done.
			Finally, note one can 'protect' a relator R which should not be deleted by replacing 
		R with R^n, n > 1, in the initial input presentation.
	**********************************************************************************************/				
	...
	if(F2 && NXMG[0] && Batch == FALSE) printf(" FGR %d",NXMG[0]);
	...
	while(1)
				...
				if(F2)
					...
					if(NumRelatorsChecked == 1)
						...
						if(NumPrimitivesFound == 1)
							...
							if(Batch == 15 && B14B15PrintPres == TRUE && H_Results != NULL)
								fprintf(H_Results,"\n\n%s \nRelators checked 1, Primitives 1",PresName);
							}
						else
							{
							printf("\n Heegaard checked 1 relator. It was not primitive.");
							if(Batch == 15 && B14B15PrintPres == TRUE && H_Results != NULL)
								fprintf(H_Results,"\n\n%s \nRelators checked 1, Primitives 0",PresName);
							}
						}
					else
						{
		...
		NumMissingGenerators 	= NXMG[ReadPres];
		Vertices 				= 2*NumGenerators;
		NumRelators 			= 1;
		NumRelatorsChecked ++;
		jj = CheckPrimitivity( );
		...
		if(jj == 1)
			...
			*************************************************************************/
			...
			if(NumGenerators == 1)
				{
				NXMG[NumFilled] = NumMissingGenerators;
			...
			if(Micro_Print == 1 && NumEmptyRelators)
				{
			...
			if(Batch != 4 && On_File() == NumFilled)
				...
				if(NXMG[NumFilled - 1] && Batch == FALSE) printf(" FGR %d",NXMG[NumFilled - 1]);
				if(F2 == FALSE) Batch = FALSE;
				}
			}
		}
	END:
		...
		if(F2)
			{
			for(ii = 0; ii < NumFilled; ii++) if(NXMG[ii])
				{
				if(Batch == FALSE) printf("\n");
...
int Reduce_The_Initial_Presentation_To_Minimal_Length_SPC()
{
    int   	i,
    		SNumGenerators;
    ...
    if(Micro_Print == 1) Report(0,0,0,0,0,0,0,0,1,NULL); 
    return(0);        
}
...
{        
    /******************************************************************************************
    	...
    	relators, then reduces the number of generators and the number of relators.
    ******************************************************************************************/
...
int Report_SPC(int *Table1)
{
    ...
    ******************************************************************************************/  
    ...
    if(NumFilled == 0) return(0);
    	    
    if(Batch == 0) for(n = 0; n < NumFilled; n++)
        {
        i = Table1[n];
        NumRelators = NR[i];
        Length = SURL[i];
        ...
        printf("FP  FGR %d ", NXMG[i]);
        for(j = 1; j <= NumRelators; j++) printf("\n    %s",*SUR[i][j]);
        } 
        
    if(Batch == 14)
    	{
    	i = Table1[NumFilled - 1];
    	NumGenerators 	= NG[i];
    	NumRelators 	= NR[i];
    	Length 			= SURL[i];
    	printf("\n\n%s Gen %d, Rel %d, L %lu",PresName,NumGenerators,NumRelators,Length);
		for(j = 1; j <= NumRelators; j++) printf("\n    %s",*SUR[i][j]);
		if(B14B15PrintPres && H_Results != NULL)
			{
			fprintf(H_Results,"\n\n%s Gen %d, Rel %d, L %lu ",PresName,NumGenerators,NumRelators,Length);
			fprintf(H_Results,"FP FGR %d",NXMG[i]);
			for(j = 1; j <= NumRelators; j++) fprintf(H_Results,"\n    %s",*SUR[i][j]);
			}	    			
    	}                       
    return(0);    
}

int Report_Sorted_Readable(int *Table1)
{
    /******************************************************************************************
        Report_Sorted_Readable() is an output routine called on termination of the sorting
        routine which sorts the presentations presently in memory. It prints sorted
        presentations to the terminal in Heegaard_Input readable form.
    ******************************************************************************************/  
    ...
    if(!Batch) for(n = 0; n < NumFilled; n++)
        {
        i = Table1[n];
        NumRelators = NR[i];
