...
/****************************** Functions in Heegaard20.c **********************************
L  88 Genus_Two_Meridian_Reps(int OrbitNum, int Flag)
L 330 Genus_Two_Meridian_Reps_Sub(unsigned char* ptr1, unsigned char* ptr2)
...
int Is_Knot_Relator(void)
{
	...
	if( (r = Genus_Two_Meridian_Reps(1,1)) )
		{
		if(r == FATAL_ERROR || r == NON_PLANAR) return(2);
...
if(Flag == -1) goto _ALREADY_HAVE_DIAGRAM;    
    ...
    if(Length < SLength)
    	{
    	printf("\n R did not have minimal length. Automorphism(s) reduced R to: ");
    	printf("%s",(char *) *Relators[1]);
    	}
...
_GET_DIAGRAM:
    ...
    switch(r)
        ...
        case NON_UNIQUE_3:
        case NON_UNIQUE_4:
         	printf("\n\nCan't find 'distinguished-slopes' unless H[R] is uniquely determined by R. Here R is: ");
        	printf("%s",(char *) *Relators[1]);
        	TestRealizability1 = FALSE;
            return(r);
    ...
    if(LTRV == 2)
    	{
    	if(Flag != -1) printf("\n");
    	printf("\n Note: The Whitehead graph of R had a separating pair of vertices.");
    	printf("\n Performed some level-transformations on R to get R': ");
    	printf("R' = %s",(char *) *Relators[1]);
    	}
                                                    
    if(Sep_Surface() > 1) 		/***** H[R] has two boundary components. *****/
    	{
    	if(LTRV == 2)
    		printf("\n\nR' is a separating curve. Can't find M1 and M2 when R' is separating.");
    	else	
			printf("\n\nR is a separating curve. Can't find M1 and M2 when R is separating.");
    	return(67);    
    	}
    ...
    ******************************************************************************************/
    ...
    if(Flag > 0)
    	...
    	if(LTRV == 2) 
    		...
    		printf("\n\nCanonical 'distinguished-slope' reps M1 and M2 of R'.");
    		}
    	else
    		{
    		if(Batch == FALSE) printf("\n\n");
    		printf("Canonical 'distinguished-slope' reps M1 and M2 of R.");
    		}
    	}
    if(!Flag)	
    	{
    	printf("\n\nCanonical 'distinguished-slope' reps M1 and M2 of the Rep Pres R of Orbit %d:", OrbitNum);
    	printf("\nR  = %s L = %lu",(char *) *Relators[1], Length);
    	}
    if(Batch == 2 && H_Results != NULL)
    	{
    	fprintf(H_Results,"\n\n%s",PresName);
    	fprintf(H_Results,"\n%s",(char *) *Relators[1]);
    	}
