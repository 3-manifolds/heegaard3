...
/********************************* Functions in Heegaard21.c *******************************
L 975 Check_If_Fibered(unsigned char* s);
L 752 CheckPolySymmetry(int *PolyCoeffs, unsigned int SizeCoeffArray);
L 783 FiberedCheck(void);
L  39 AlexanderPolynomial(void);
L 877 AlexanderPolynomial_Freely_Reduce(char* TransR, unsigned int LengthT);
L 212 AlexanderPolynomial_GCD(unsigned int AP, unsigned int AQ);
L 445 ComputeAlexanderPolynomial(void);
L 283 ComputePrimitives(unsigned int p, unsigned int q, unsigned int r, unsigned int s);
L 922 CountMaxMins(char* TransR);
********************************************************************************************/	
...
int AlexanderPolynomial(unsigned char* u)
{
	...
	/**********************************************************************************************
		...
		If G = < A,B | R > is a 2-generator, 1-relator presentation with relator R and R does not
		abelianize to (0,0), this routine will compute and display the Alexander polynomial of R, 
		and determine if the commutator subgroup of G is finitely generated.
	**********************************************************************************************/		
	...
	if(AP == 0 && AQ == 0)
		{
		printf("\nThe input relator has trivial abelianization. Can't compute ∆(t).");
		if(Batch == 1 && H_Results != NULL) fprintf(H_Results,"\nThis relator has trivial abelianization. Can't compute ∆(t).");
		return(0);
		}
		
	m = AlexanderPolynomial_GCD(AP,AQ);
	if(m > 1)
		{
		AP = AP/m;
		AQ = AQ/m;
		}
	AlexanderPolynomial_GCD(AP,AQ);
...
**********************************************************************************************/			
	...
	/*****************************************************************************************
		...
		Alexander polynomial. Initialize the array by setting all entries to 0.
	*****************************************************************************************/
	...
	******************************************************************************************/
	...
	if(PolyCoeffs[-Offset] == 1)
		...
		{
		printf("\n\nThe commutator subgroup of the group G = < A,B | R > is infinitely generated.");
...
int FiberedCheck(void)
{
	...
	*q = EOS;
	...
	if(CountMaxMins((char *) TransR) == 1)
		...
		if(Deg_Delta_t == 0)
			{
			printf("\n\nThe commutator subgroup of the group G = < A,B | R > is trivial.");
			if(Batch == 1 && H_Results != NULL) 
				fprintf(H_Results,"\n\nThe commutator subgroup of the group G = < A,B | R > is trivial.");
			}		
		if(Deg_Delta_t > 0)
			{
			printf("\n\nThe commutator subgroup of the group G = < A,B | R > is free of rank %ld.",Deg_Delta_t);
			if(Batch == 1 && H_Results != NULL) 
				fprintf(H_Results,"\n\nThe commutator subgroup of the group G = < A,B | R > is free of rank %ld.",Deg_Delta_t);
			}
		if(TransR) DisposePtr((char *) TransR);
...
int Check_If_Fibered(unsigned char* s)
{
	char			x;
	...
	/*******************************************************************************************
		This routine checks if H[R] is fibered when H is genus two, and R is non-separating.
		It returns 1 if H[R] is fibered, 0 if H[R] is not fibered, and 2 on some error.
	********************************************************************************************/
	
	if(NumGenerators != 2 || NumRelators != 1) return(2);
	...
	if(NumA == 0 && NumB == 0)
		{
		printf("\nThe input relator has trivial abelianization. Can't determine if H[R] is fibered.");
		return(2);
		}
		
	m = AlexanderPolynomial_GCD(abs(NumA),abs(NumB));
	if(m > 1)
		{
		NumA = NumA/m;
		NumB = NumB/m;
	...
	if(ThetaA == 0 || ThetaB == 0)
		{
		if(NumMax != 2 || NumMin != 2) 
			{
			printf("\n\nThe commutator subgroup of the group G = < A,B | R > is infinitely generated.");
			return(0);
			}
		/* Check if R is AA, aa, BB or bb. */	
		t = s;
		t++;
		u = t;
		u++;
		...
		if(NumMax == 2 && NumMin == 2)	printf("\n\nThe commutator subgroup of the group G = < A,B | R > is infinitely generated.");	
		return(1);
		}
	
	if(NumMax == 1 && NumMin == 1) 
		{
		printf("\n\nThe commutator subgroup of the group G = < A,B | R > is finitely generated."); 
		return(1);
		}
	else 
		{
