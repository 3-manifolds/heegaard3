...
/****************************** Functions in Heegaard 4.c **********************************
L 1477 Compare(unsigned char *p)
L 1502 Compare_Str(unsigned char *S1,unsigned char *S2,unsigned long length)
L 2226 Debug(void)
L 1679 Delete_Redundant_Relators(void)
L   85 Diagram_1(void)
L  189 Diagram_2(void)
L  290 Diagram_3(void)
L  495 Diagram_4(void)
L  741 Diagram_5(void)
L 1255 Diagram_6(void)
L 1294 Diagram_7(void)
L   28 Diagram_Main(void)
L 2046 Fill_DRA(void)
L 2098 GCD(unsigned int p,unsigned int q)
L 1552 Get_Bdry_Comps(int Print,int Where,unsigned int WhichPres)
L 2146 Inverse(unsigned char *p)
L 1804 MG_Bdry_Comp_Data(unsigned int WhichPres)
L 2174 Non_Unique(void)
L  766 Offset(void)
L 1215 OffsetSub(unsigned int i,unsigned int p,unsigned int q,unsigned int a,unsigned int b,unsigned int c)
L 1992 Sep_Surface(void)		
********************************************************************************************/

unsigned int Diagram_Main(void)
{
    /******************************************************************************************
    ...
    ******************************************************************************************/
    ...
    if(NGV2) switch(Valence_Two(FALSE))
        ...
        case V2_ANNULUS_EXISTS:
            if(TestRealizability3 || SPM_Flag) return(V2_ANNULUS_EXISTS);
...
int Diagram_1(void)
{
    ...
    ******************************************************************************************/    
    ...
    if(TestRealizability1 || SPM_Flag)
        {
...
{        
    ...
    ******************************************************************************************/                 
    ...
    if(SPM_Flag) for(i = NumGenerators - 1; i >= 0; i--)
    	{
    	/************************************************************************ 
    		Check if a generator appears only with exponents |e| with |e| > 2. 
    					Generally this implies Not Unique! 									
    	************************************************************************/
    	p = EXP[i];
    	if(p[2] > 2 && p[1] == 0) return(1);  		
    	}  
        
    if(TestRealizability1 || SPM_Flag) for(i = NumGenerators - 1; i >= 0; i--)
        {
        p = EXP[i];
        if(p[0])
            {
...
int Delete_Redundant_Relators(void)
{
    /******************************************************************************************
    ...
    ******************************************************************************************/
    ...
    /* Try setting UDV[ReadPres] = DONE so we won't process it more than once unless NunGenerators = 2. */
    
    if(NumGenerators != 2 && UDV[ReadPres] < DONE) UDV[ReadPres] = DONE;
...
void Debug(void)
{
    ...
    ******************************************************************************************/
                    ...
                    for(i = 0; i < NumGenerators; i++) printf("%4u",PG[i]);
                    printf("\n");
