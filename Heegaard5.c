...
/****************************** Functions in Heegaard5.c ***********************************
L  590 Canonical_Rewrite(unsigned char ***MyRelators,int Annulus_Exists,int ReportSymmetries)
L 2678 CP_Check_Simple_Paths(unsigned int NCF,int* My_PathsInCircuit,int NumPaths,
	   unsigned char** MyPP,unsigned char** MyPM)
L 2541 CP_Concatenate_Paths(unsigned char **PM, unsigned char **PP,int NumPaths)
L 3030 CP_Do_Aut(unsigned int Source,char Flag)
L 2779 CP_Fill_AA(char Flag)
L 2833 CP_Find_Primitives(char Flag)
L 1508 Final_Rewrite(unsigned char	***MyRelators)
L 1857 Find_Cancellation_Paths(int F1,int F2,int Pres)
L 1722 Find_Symmetries(int Flag)
L  545 Freely_Reduce_Nr(void)
L  517 Inverse_Nr(unsigned char *p)	
L  347 Long_Mult(unsigned char,unsigned char,unsigned int,unsigned int,unsigned int,
	   unsigned int,unsigned int,unsigned int,unsigned int,long,unsigned long)
L   23 New_Relator(int F1)
L 1812 Report_Symmetries(unsigned char *T2,int j,int NumDups)
********************************************************************************************/
...
int New_Relator(int F1)
{
	...
	******************************************************************************************/							
	...
	/******************************************************************************************
		...
		bandsums is determined by this parameter. Note NumPaths = NumEdges <= 6(NumGens - 1).
	******************************************************************************************/	
	
	if(GoingDown)
		Depth = NumEdges;
	else
		Depth = NumEdges - UDV[ReadPres]/2;
	
	if(BreadthFirstSearch) Depth = NumEdges - UDV[ReadPres]/2;
	
	if(Depth < NumEdges/4) Depth = NumEdges/4;
	...
	for(ss = 2*NumEdges; ss > 0; ss --)
		...
		**************************************************************************************/		
		...
		if(uu >= Depth || (Minimum < 0L && GoingDown && (uu << 2) >= Depth) ||
			(uu >= Depth && Minimum == 0L && GoingDown && Boundary))
			{			
			/**********************************************************************************
...
void Inverse_Nr(unsigned char *p)
{
	/******************************************************************************************
...
void Find_Cancellation_Paths(int F1,int F2,int Pres)
{
	...
	******************************************************************************************/
	...
	printf("\n\nIn each path above, X|Y indicates relators X and Y lie to the left and right respectively of the oriented path.\n");
	for(i = 1; i <= NumFaces; i++)
		{
		pp = P_From_Face[i];
		printf("\nPaths from F%2d: ",i);
					...
					switch(k)
						...
						case 3:
							...
							for(j = 1; j <= NumPathsInCircuit; j++)
								{
								printf("F%d,",FacesVisitedList[j]);
								kk = PathsInCircuit[j];
								if(kk > 0) printf("P%d,",kk);
								else printf("-P%d,",-kk);
								}
...
void CP_Concatenate_Paths(unsigned char **PM, unsigned char **PP,int NumPaths)
{
	...
	printf("\nCurrently the path is %s.",(char *) *Relators[1]);
		...
		case 1: 
			...
			********************************************************************************************/	
			...
			for(i = j = k = 0; i < SNumGenerators; i++,j+=2)
				...
				if(C[j] == 1) 
					{
					printf("\n %3u) %s is primitive.",NCF,(char *) *Relators[2]);
					NumGenerators = SNumGenerators;
					return(1);
					}
				if(C[j]) k++;
			...
			if(k == 1) 
				{
				printf("\n %3u) %s is a proper-power.",NCF,(char *) *Relators[2]);
				NumGenerators = SNumGenerators;
				return(2);				
				}
			if(k < SNumGenerators)
				{
				printf("\n %3u) %s has rank %d.",NCF,(char *) *Relators[2],k);
				NumGenerators = SNumGenerators;
				return(3);				
				}
			if(k == SNumGenerators)
