...
/****************************** Functions in Heegaard7.c ************************************
L  370 Annulus(unsigned int V3,unsigned int V4,int TheComp,int F1)
L  339 Been_Seen(void)
L 1776 Count_Sep_Pairs(unsigned int Num_Saved_LPres)
L 1120 Delete_Trivial_Generators(int Test)
L  725 Do_Aut_L(void)
L  834 Do_Aut_L_Long(int V1,int V2,int TheComp)
...
int Level_Transformations(int F1,int F2,int F3)
{
	...
	******************************************************************************************/	
	...
	if(Num_Saved_LPres == 0)
		{
		for(i = 0; i < Vertices; i++) ZZ[i] = 0;
		if(Connected_(0,0) == FALSE) return(7);
		}
	...
	if((MyChar = mykbhit()))
		...
		if(TestRealizability1)
			{
			if(MyChar == 's')
				printf("\n	Status: In 'Level_Transformations()'. Num_Saved_LPres %u, Num_Level_Slides %lu",Num_Saved_LPres,Num_Level_Slides - SNum_Level_Slides); 				
			if(MyChar == ' ')
				{
				printf("\n	Status: In 'Level_Transformations()'. Num_Saved_LPres %u, Num_Level_Slides %lu",Num_Saved_LPres,Num_Level_Slides - SNum_Level_Slides);				
...
int Annulus(unsigned int V3,unsigned int V4,int TheComp,int F1)
{
	...
	*q++ = '@';
	...
	if(i == NumFilled)
		...
		if(TestRealizability1 == 3 && Batch == FALSE)
			{
			if(Separating) 
				printf("\n                    There is a separating annulus in diagram %u.",NumFilled);
			else
...
RETRY:
	...
	for(i = 1; i <= NumRelators; i++) if(GetHandleSize((char **) Relators[i]) == 2)
		...
		**********************************************************************************/
		...
		if(Micro_Print == 1) for(j = 1,Length = 0L; j <= NumRelators; j++)
...
int Level_Trans_Reset(unsigned int MyNum,unsigned int V3,unsigned int V4)
{
	...
	**********************************************************************************/	
	...
	if(Micro_Print == 1) Micro_Print_Level_Transformations_Reset(MyNum);	
