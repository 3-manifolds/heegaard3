...
L  608 Valence_Two_Annulus(void)
********************************************************************************************/
...
void Test_Sub_Str(unsigned int j)
{
	/*****************************************************************************************
		...
		of "offset" is correct, OSA[2j] or OSB[2j]. It is not hard to see that if the
		presentation in Relators[] is realizable, (and there is no annulus present)
	...
	if(MissingGens)
		...
		if(Length == 0L)
			...
			if(BDY[ReadPres] == FALSE)
				...
				if(UDV[NumFilled - 1] < DONE) UDV[NumFilled - 1] = DONE;
			return(NO_ERROR);	
			}
		...
		NumFilled -= 2;
		i = On_File();
		if(i < NumFilled) Daughters[NumFilled] = i;
		NumFilled += 2;
		}
